...
// Display colors
// Marked static so this header stays safe if it's ever #included from more
// than one .cpp file - newer GCC toolchains (used by the ESP32-S3 Arduino
// core) will throw a "multiple definition" linker error otherwise.
...
// Space between eyes
// Declared here (rather than down in the "BOTH EYES" block below, as in
// the original) because eyeLxDefault/eyeRxDefault right below need it -
// default member initializers run in declaration order, so reading it
// before its own initializer had run always produced 0 (both eyes stuck
// touching at startup) regardless of the value set here.
